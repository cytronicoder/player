...
async function createWindow() {
  ...
  if (process.env.NODE_ENV === 'development') {
    // In development, expect a Vite dev server at localhost:5173
    try {
      await mainWindow.loadURL('http://localhost:5173');
    } catch (e) {
      console.error('Failed to load dev server URL', e);
      dialog.showErrorBox('Dev Server Missing', 'Unable to load http://localhost:5173. Start the renderer dev server (pnpm run dev) or run `pnpm run dev:all`.');
      app.quit();
    }
...
async function parsePlaylistFolder(folderPath: string, folderName: string): Promise<Playlist> {
  ...
  if (imageFile) {
    coverImage = path.join(folderPath, imageFile); // In real app, handle protocol
  }
